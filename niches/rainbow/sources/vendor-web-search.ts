/**
 * Vendor Web Search Adapter — Rainbow niche
 * Adapter interface version: 1
 *
 * Uses the web_search tool (Serper.dev) to find LGBTQ+ friendly local wedding
 * vendors by category and location. Results are saved to the Vendors database.
 *
 * This adapter is invoked on-demand from the research chatbot when the
 * couple asks to find vendors in their area.
 */
  ...
  implements DataAdapter<VendorSearchRaw, VendorRow, VendorSearchCriteria>
{
  ...
  readonly description =
    "Finds LGBTQ+ friendly local wedding vendors using web search, by category and location.";
  ...
  async *fetch(
    ...
    const query = `LGBTQ+ friendly ${criteria.category} wedding ${criteria.location}`;
    const maxResults = Math.min(criteria.maxResults ?? 10, 20);

    console.info("[rainbow/vendor-web-search] fetch:start", { query, maxResults });
    ...
    console.info("[rainbow/vendor-web-search] fetch:results", { count: results.length });
...
function toNotionCategory(category: string): string {
  ...
  if (lower.includes("dress") || lower.includes("attire") || lower.includes("suit")) return "Attire";
