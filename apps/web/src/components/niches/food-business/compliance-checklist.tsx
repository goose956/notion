...
export function FDComplianceChecklist({
  criteria,
  licencesDb,
  onRowAdded,
}: {
  criteria:   Record<string, unknown> | null;
  licencesDb: WorkspaceDatabase | null;
  ...
  const foodType      = String(criteria?.["food-type"]      ?? "").trim();
  const premisesType  = String(criteria?.["premises-type"]  ?? "").trim();
  const country       = String(criteria?.["country"]        ?? "United Kingdom").trim();

  async function generate() {
    if (!businessName || !foodType) return;
    setLoading(true);
    setError(null);
    setResult(null);
    setSaveResult(null);
    try {
      const res = await fetch("/api/members/fd-compliance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessName, foodType, premisesType, country }),
      });
      if (!res.ok) {
  ...
  async function saveToNotion() {
    ...
    try {
      for (const item of result.items) {
        ...
        const propertyTypes = {
          "Licence": "title", "Category": "select", "Status": "select",
          "Authority": "rich_text", "Cost": "rich_text", "Notes": "rich_text",
        };
        const res = await fetch("/api/members/workspace", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ databaseId: licencesDb.notionId, properties, propertyTypes }),
        });
        if (res.ok) {
          const data = await res.json() as { pageId?: string };
          if (data.pageId) { onRowAdded(licencesDb.notionId, { pageId: data.pageId, properties }); added++; }
        }
      }
  ...
  // Get existing licences from the DB to show status
  const existingLicences = new Set(
    (licencesDb?.rows ?? []).map((r) => asText(r.properties["Licence"]).toLowerCase()),
  );

  const missingSetup = !businessName || !foodType;
    ...
    <div style={{ maxWidth: "720px", fontFamily: N_FONT }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ margin: 0, fontSize: "13px", color: N_MUTED, lineHeight: 1.6 }}>
          Generate a tailored compliance checklist — every licence, registration, certificate and insurance you need to open legally, specific to your food type and country.
        </p>
      </div>
      ...
      {missingSetup && (
        <div style={{ padding: "12px 14px", borderRadius: "8px", background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.22)", fontSize: "13px", color: "#92400e", marginBottom: "20px" }}>
          Complete your onboarding (business name + food type) to generate your compliance checklist.
        </div>
      )}
      ...
      {!missingSetup && (
        <div style={{ padding: "12px 16px", borderRadius: "10px", background: "rgba(55,53,47,0.04)", border: `1px solid ${N_BORDER}`, marginBottom: "20px", fontSize: "13px", color: N_MUTED }}>
          <strong style={{ color: N_FG }}>{businessName}</strong> · {foodType}{premisesType ? ` · ${premisesType}` : ""} · {country}
        </div>
      )}
        ...
        <div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: isActive ? (c?.color ?? "#b91c1c") : N_SUBTLE,
                    fontSize: "12px",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px" }}>
            {filtered.map((item) => {
              ...
              const alreadyIn = existingLicences.has(item.name.toLowerCase());
              return (
                <div
                  key={item.name}
                  style={{
                    ...
                    background: alreadyIn ? "rgba(5,150,105,0.04)" : "white",
                    overflow: "hidden",
                  }}
                >
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            <button
              ...
              style={{
                ...
                border: `1px solid ${N_BORDER}`, background: "white",
                color: N_FG, fontSize: "14px", fontWeight: 500, cursor: loading ? "default" : "pointer", fontFamily: N_FONT,
              }}
            >
              Regenerate
            </button>
