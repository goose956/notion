...
export function FDPlanBuilder({
  ...
  async function generate() {
    ...
    setSaveResult(null);
    try {
      const res = await fetch("/api/members/fd-plan", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          businessName,
          foodType,
          premisesType:  criteria?.["premises-type"]         ?? "",
          goal:          criteria?.["primary-goal"]          ?? "",
          country:       criteria?.["country"]               ?? "United Kingdom",
          avgSpend:      criteria?.["avg-spend"]       ? Number(criteria["avg-spend"])        : undefined,
          foodCostPct:   criteria?.["food-cost-pct"]   ? Number(criteria["food-cost-pct"])    : undefined,
    ...
    <div style={{ maxWidth: "720px", fontFamily: N_FONT }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ margin: 0, fontSize: "13px", color: N_MUTED, lineHeight: 1.6 }}>
          Generate each section of your food business plan. AI writes it using your business details — you review and save it to Notion.
        </p>
      </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? "#b91c1c" : N_SUBTLE,
                  fontSize: "13px",
        ...
        <div>
          <div style={{ borderRadius: "12px", border: `1px solid ${N_BORDER_MED}`, background: "white", overflow: "hidden", marginBottom: "16px" }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${N_BORDER}`, background: ACCENT_LIGHT, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <p style={{ margin: 0, fontSize: "13px", fontWeight: 700, color: "#b91c1c" }}>{result.section}</p>
