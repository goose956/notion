...
type DocType = "All" | "Idea Score" | "Project Plan" | "Launch Kit";
...
function DocRow({ row }: { row: WorkspaceRow }) {
  ...
  function exportPdf() {
    ...
    w.document.write(`<html><head><title>${title}</title><style>
      ...
      h1{font-size:20px;}strong{color:#65a30d;}pre{white-space:pre-wrap;font-family:inherit;}
    </style></head><body>
      <h1>${title}</h1>
      ${project ? `<p style="color:#666;font-size:13px;">${project}</p>` : ""}
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
      <div onClick={() => setExpanded((x) => !x)} style={{ padding: "10px 16px", display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
        ...
        {project && <span style={{ fontSize: "11px", color: N_MUTED, flexShrink: 0 }}>{project}</span>}
...
export function VibeCoderLibrary({ databases }: { databases: WorkspaceDatabase[] }) {
  const [filter, setFilter] = useState<DocType>("All");

  const docsDb  = databases.find((d) => d.nicheId === "vibe-coder" && d.dbId === "documents") ?? null;
  ...
  function exportAll() {
    ...
    const body = visible.map((row) => {
      ...
      const project = asText(row.properties["Project"]);
      const content = asText(row.properties["Content"]);
      return `<h2>${title}${project ? ` — ${project}` : ""}</h2><pre>${content.replace(/</g, "&lt;")}</pre><hr/>`;
    }).join("\n");
    w.document.write(`<html><head><title>Vibe Coder Documents</title><style>
      body{font-family:Georgia,serif;max-width:800px;margin:40px auto;padding:0 24px;color:#111;line-height:1.8;}
      h2{color:#65a30d;font-size:16px;}pre{white-space:pre-wrap;font-family:inherit;font-size:13px;}hr{border:none;border-top:1px solid #ddd;margin:24px 0;}
    </style></head><body>${body}</body></html>`);
    w.document.close(); w.print();
  }

  const FILTERS: DocType[] = ["All", "Idea Score", "Project Plan", "Launch Kit"];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", fontFamily: N_FONT }}>
      ...
      {visible.length === 0 ? (
        <div style={{ padding: "32px", textAlign: "center", borderRadius: "12px", border: `1px dashed ${ACCENT_BORDER}`, background: ACCENT_LIGHT }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "13px", color: N_MUTED }}>Use the Idea Scorer, Project Planner or Launch Kit to generate and save documents.</p>
        </div>
      ) : (
