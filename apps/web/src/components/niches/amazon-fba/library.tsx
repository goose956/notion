...
type DocType = "All" | "Research Report" | "Listing" | "Supplier Brief";
const TYPE_COLORS: Record<string, string> = { "Research Report": "#2563eb", "Listing": "#ea580c", "Supplier Brief": "#7c3aed" };
...
function DocRow({ row }: { row: WorkspaceRow }) {
  ...
  function exportPdf() {
    ...
    w.document.write(`<html><head><title>${title}</title><style>
      ...
      h1{font-size:20px;}strong{color:#c2410c;}pre{white-space:pre-wrap;font-family:inherit;}
    </style></head><body>
      <h1>${title}</h1>
      ${product ? `<p style="color:#666;font-size:13px;">${product}</p>` : ""}
...
export function FBALibrary({ databases }: { databases: WorkspaceDatabase[] }) {
  ...
  function exportAll() {
    ...
    const body = visible.map((row) => {
      ...
      const product = asText(row.properties["Product"]);
      const content = asText(row.properties["Content"]);
      return `<h2>${title}${product ? ` — ${product}` : ""}</h2><pre>${content.replace(/</g, "&lt;")}</pre><hr/>`;
    }).join("\n");
    w.document.write(`<html><head><title>Amazon FBA Documents</title><style>
      body{font-family:Georgia,serif;max-width:800px;margin:40px auto;padding:0 24px;color:#111;line-height:1.8;}
      h2{color:#c2410c;font-size:16px;}pre{white-space:pre-wrap;font-family:inherit;font-size:13px;}hr{border:none;border-top:1px solid #ddd;margin:24px 0;}
    </style></head><body>${body}</body></html>`);
    w.document.close(); w.print();
  }

  const FILTERS: DocType[] = ["All", "Research Report", "Listing", "Supplier Brief"];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", fontFamily: N_FONT }}>
      ...
      {visible.length === 0 ? (
        <div style={{ padding: "32px", textAlign: "center", borderRadius: "12px", border: `1px dashed ${ACCENT_BORDER}`, background: ACCENT_LIGHT }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "13px", color: N_MUTED }}>Use Product Research, Listing Writer or Supplier Brief to generate and save documents.</p>
        </div>
      ) : (
