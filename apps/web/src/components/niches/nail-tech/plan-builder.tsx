...
export function NailPlanBuilder({
  criteria,
  documentsDb,
  onRowAdded,
}: {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/members/nail-plan", {
        method: "POST",
    ...
    <div style={{ maxWidth: "720px", fontFamily: N_FONT }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ margin: 0, fontSize: "13px", color: N_MUTED, lineHeight: 1.6 }}>
          Generate each section of your beauty business plan. AI writes it using your specific details — review and save to Notion.
        </p>
      </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? "#86198f" : N_SUBTLE,
        ...
        <div>
          <div style={{ borderRadius: "12px", border: `1px solid ${N_BORDER_MED}`, background: "white", overflow: "hidden", marginBottom: "16px" }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${N_BORDER}`, background: ACCENT_LIGHT, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <p style={{ margin: 0, fontSize: "13px", fontWeight: 700, color: "#86198f" }}>{result.section}</p>
