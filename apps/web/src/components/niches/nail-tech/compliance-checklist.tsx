...
const CATEGORIES = ["Registration", "Certificate", "Insurance", "COSHH", "Patch Testing", "Inspection"] as const;
...
const CATEGORY_COLORS: Record<Category, { bg: string; color: string }> = {
  ...
  COSHH:         { bg: "rgba(245,158,11,0.09)",  color: "#92400e" },
  "Patch Testing":{ bg: "rgba(192,38,211,0.09)", color: "#86198f" },
  Inspection:    { bg: "rgba(220,38,38,0.09)",   color: "#b91c1c" },
};

export function NailComplianceChecklist({
  criteria,
  complianceDb,
  onRowAdded,
}: {
  ...
  const operationType = String(criteria?.["operation-type"] ?? "Home salon").trim();
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/members/nail-compliance", {
        method: "POST",
      ...
      setResult(await res.json() as ComplianceResult);
    } catch (err) {
    ...
    <div style={{ maxWidth: "720px", fontFamily: N_FONT }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ margin: 0, fontSize: "13px", color: N_MUTED, lineHeight: 1.6 }}>
          Every registration, certificate, insurance and legal requirement to run a nail tech or beauty business legally —
          including COSHH assessments for nail chemicals, patch testing protocols and treatment liability insurance.
        </p>
      </div>
        ...
        <div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: isActive ? (c?.color ?? "#86198f") : N_SUBTLE,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px" }}>
            {filtered.map((item) => {
              ...
              return (
                <div key={item.name} style={{ borderRadius: "12px", border: `1px solid ${N_BORDER_MED}`, background: alreadyIn ? "rgba(5,150,105,0.04)" : "white", overflow: "hidden" }}>
                  <div style={{ padding: "12px 16px" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", fontSize: "11px" }}>
                      {item.authority && <span><strong style={{ color: N_FG }}>Authority:</strong> <span style={{ color: N_MUTED }}>{item.authority}</span></span>}
                      {item.timescale && <span><strong style={{ color: N_FG }}>When:</strong> <span style={{ color: N_MUTED }}>{item.timescale}</span></span>}
                      {item.cost && <span><strong style={{ color: N_FG }}>Cost:</strong> <span style={{ color: N_MUTED }}>{item.cost}</span></span>}
                    </div>
                  </div>
                </div>
              );
