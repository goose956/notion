...
type DocType = "All" | "Episode Plan" | "Show Notes" | "Guest Outreach" | "Transcript Summary";
const TYPE_COLORS: Record<string, string> = {
  "Episode Plan":        "#7c3aed",
  "Show Notes":          "#2563eb",
  "Guest Outreach":      "#16a34a",
  "Transcript Summary":  "#d97706",
};
...
function DocRow({ row }: { row: WorkspaceRow }) {
  ...
  function exportPdf() {
    ...
    w.document.write(`<html><head><title>${title}</title><style>
      ...
      h1{font-size:20px;}strong{color:#6d28d9;}pre{white-space:pre-wrap;font-family:inherit;}
    </style></head><body>
      <h1>${title}</h1>
      ${episode ? `<p style="color:#666;font-size:13px;">${episode}</p>` : ""}
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
      <div onClick={() => setExpanded((x) => !x)} style={{ padding: "10px 16px", display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
        ...
        {episode && <span style={{ fontSize: "11px", color: N_MUTED, flexShrink: 0 }}>{episode}</span>}
        <button onClick={(e) => { e.stopPropagation(); copyText(); }} style={{ flexShrink: 0, padding: "3px 10px", borderRadius: "6px", border: `1px solid ${ACCENT_BORDER}`, background: "white", color: ACCENT_TEXT, fontSize: "11px", fontWeight: 600, cursor: "pointer" }}>Copy</button>
        <button onClick={(e) => { e.stopPropagation(); exportPdf(); }} style={{ flexShrink: 0, padding: "3px 10px", borderRadius: "6px", border: `1px solid ${ACCENT_BORDER}`, background: "white", color: ACCENT_TEXT, fontSize: "11px", fontWeight: 600, cursor: "pointer" }}>PDF</button>
...
export function PodcastLibrary({ databases }: { databases: WorkspaceDatabase[] }) {
  const [filter, setFilter] = useState<DocType>("All");

  const docsDb  = databases.find((d) => d.nicheId === "podcast" && d.dbId === "documents") ?? null;
  ...
  function exportAll() {
    ...
    const body = visible.map((row) => {
      ...
      const episode = asText(row.properties["Episode"]);
      const content = asText(row.properties["Content"]);
      return `<h2>${title}${episode ? ` — ${episode}` : ""}</h2><pre>${content.replace(/</g, "&lt;")}</pre><hr/>`;
    }).join("\n");
    w.document.write(`<html><head><title>Podcast Documents</title><style>
      body{font-family:Georgia,serif;max-width:800px;margin:40px auto;padding:0 24px;color:#111;line-height:1.8;}
      h2{color:#6d28d9;font-size:16px;}pre{white-space:pre-wrap;font-family:inherit;font-size:13px;}hr{border:none;border-top:1px solid #ddd;margin:24px 0;}
    </style></head><body>${body}</body></html>`);
    w.document.close(); w.print();
  }

  const FILTERS: DocType[] = ["All", "Episode Plan", "Show Notes", "Guest Outreach", "Transcript Summary"];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", fontFamily: N_FONT }}>
      ...
      {visible.length === 0 ? (
        <div style={{ padding: "32px", textAlign: "center", borderRadius: "12px", border: `1px dashed ${ACCENT_BORDER}`, background: ACCENT_LIGHT }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "13px", color: N_MUTED }}>Use the Episode Planner, Show Notes, Guest Outreach or Transcript Summary tools to generate and save documents.</p>
        </div>
      ) : (
