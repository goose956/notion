...
export function SHPlanBuilder({
  ...
  async function generate() {
    if (!businessName || !whatYouSell) return;
    setLoading(true);
    setError(null);
    setResult(null);
    setSaveResult(null);
    try {
      const res = await fetch("/api/members/sh-plan", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          pricePerSale:    criteria?.["price-per-sale"]  ? Number(criteria["price-per-sale"])  : undefined,
          costPerSale:     criteria?.["cost-per-sale"]   ? Number(criteria["cost-per-sale"])   : undefined,
          fixedCosts:      criteria?.["fixed-monthly-costs"] ? Number(criteria["fixed-monthly-costs"]) : undefined,
          targetCustomers: criteria?.["target-customers"] ? Number(criteria["target-customers"]) : undefined,
          extraContext,
        }),
      });
      if (!res.ok) {
    ...
    <div style={{ maxWidth: "720px", fontFamily: N_FONT }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ margin: 0, fontSize: "13px", color: N_MUTED, lineHeight: 1.6 }}>
          Generate each section of your business plan. AI writes it using your business details — you review and save it to Notion.
        </p>
      </div>
      ...
      {missingSetup && (
        <div style={{ padding: "12px 14px", borderRadius: "8px", background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.22)", fontSize: "13px", color: "#92400e", marginBottom: "20px" }}>
          Complete your onboarding (business name + what you sell) to generate plan sections.
        </div>
      )}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? "#c2410c" : N_SUBTLE,
                  fontSize: "13px",
      ...
      <button
        ...
        style={{
          ...
          fontSize: "14px",
          fontWeight: 600,
          cursor: loading || missingSetup ? "default" : "pointer",
          fontFamily: N_FONT,
          marginBottom: "28px",
        }}
      >
        ...
        <div>
          <div style={{ borderRadius: "12px", border: `1px solid ${N_BORDER_MED}`, background: "white", overflow: "hidden", marginBottom: "16px" }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${N_BORDER}`, background: ACCENT_LIGHT, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <p style={{ margin: 0, fontSize: "13px", fontWeight: 700, color: "#c2410c" }}>{result.section}</p>
