...
type DocType = "All" | "Proposal" | "Invoice" | "Outreach";
const TYPE_COLORS: Record<string, string> = { Proposal: "#4f46e5", Invoice: "#16a34a", Outreach: "#7c3aed" };
...
function DocRow({ row }: { row: WorkspaceRow }) {
  ...
  function exportPdf() {
    ...
    w.document.write(`<html><head><title>${title}</title><style>
      ...
      h1{font-size:20px;}strong{color:#3730a3;}pre{white-space:pre-wrap;font-family:inherit;}
    </style></head><body>
      <h1>${title}</h1>
      ${client ? `<p style="color:#666;font-size:13px;">${client}</p>` : ""}
...
export function FreelancerLibrary({ databases }: { databases: WorkspaceDatabase[] }) {
  ...
  function exportAll() {
    ...
    w.document.write(`<html><head><title>Freelancer Documents</title><style>
      body{font-family:Georgia,serif;max-width:800px;margin:40px auto;padding:0 24px;color:#111;line-height:1.8;}
      h2{color:#3730a3;font-size:16px;}pre{white-space:pre-wrap;font-family:inherit;font-size:13px;}hr{border:none;border-top:1px solid #ddd;margin:24px 0;}
    </style></head><body>${body}</body></html>`);
    w.document.close(); w.print();
  }

  const FILTERS: DocType[] = ["All", "Proposal", "Invoice", "Outreach"];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", fontFamily: N_FONT }}>
      ...
      {visible.length === 0 ? (
        <div style={{ padding: "32px", textAlign: "center", borderRadius: "12px", border: `1px dashed ${ACCENT_BORDER}`, background: ACCENT_LIGHT }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "13px", color: N_MUTED }}>Use Proposal Writer, Invoice Builder or Outreach Writer to generate and save documents.</p>
        </div>
      ) : (
