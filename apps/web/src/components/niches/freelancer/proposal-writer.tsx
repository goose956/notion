...
const PROJECT_TYPES = ["Website Design", "Website Development", "Branding & Logo", "Copywriting", "SEO & Content", "Social Media Management", "Marketing Strategy", "Photography", "Video Production", "Graphic Design", "Consulting", "Virtual Assistant", "Other"];
const TIMELINES     = ["1 week", "2 weeks", "3–4 weeks", "1–2 months", "2–3 months", "3–6 months", "Ongoing"];
...
export function FreelancerProposalWriter({
  criteria,
  documentsDb,
  onRowAdded,
}: {
  ...
  const [result,  setResult]  = useState<{ proposal: string; title: string; clientName: string } | null>(null);
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/members/freelancer-proposal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ clientName, projectType, scope, deliverables, timeline, budget, yourName, yourService, country: String(criteria?.["country"] ?? "") }),
      });
      if (!res.ok) throw new Error(((await res.json()) as { error?: string }).error ?? "Failed");
      setResult(await res.json() as { proposal: string; title: string; clientName: string });
    } catch (err) {
    ...
    try {
      const res = await fetch("/api/members/workspace", {
        ...
        body: JSON.stringify({
          ...
          properties:    { Title: result.title, Type: "Proposal", Client: result.clientName, Content: result.proposal },
  ...
  function exportPdf() {
    ...
    w.document.write(`<html><head><title>${result.title}</title><style>
      ...
      h1{font-size:22px;margin-bottom:4px;}strong{color:#3730a3;}
      pre{white-space:pre-wrap;font-family:inherit;}
    </style></head><body><h1>${result.title}</h1><pre>${result.proposal.replace(/</g, "&lt;")}</pre></body></html>`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px", fontFamily: N_FONT, maxWidth: "720px" }}>
      <div>
        <h2 style={{ margin: "0 0 4px", fontSize: "17px", fontWeight: 700, color: N_FG }}>Proposal Writer</h2>
        <p style={{ margin: 0, fontSize: "13px", color: N_MUTED }}>Generate a full professional project proposal — ready to send to your client.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
        ...
        <div style={{ gridColumn: "1 / -1" }}>
          {label("Project Scope")}
          <textarea style={ta} value={scope} onChange={(e) => setScope(e.target.value)} placeholder="Describe what the project involves — what problem are you solving for them?" />
        </div>
        <div style={{ gridColumn: "1 / -1" }}>
          {label("Key Deliverables")}
          <textarea style={{ ...ta, minHeight: "60px" }} value={deliverables} onChange={(e) => setDeliverables(e.target.value)} placeholder="e.g. 5-page website, logo in 3 formats, 2 rounds of revisions" />
        </div>
        <div>
        ...
        <div>
          ...
          <input style={inp} value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="e.g. £2,500 or $3,000" />
        </div>
      </div>
      ...
      <button onClick={generate} disabled={loading} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", padding: "10px 20px", borderRadius: "8px", border: "none", background: loading ? "#e0e7ff" : ACCENT, color: "white", fontWeight: 700, fontSize: "14px", cursor: loading ? "not-allowed" : "pointer" }}>
        {loading ? <><Loader2 size={15} style={{ animation: "spin 1s linear infinite" }} /> Writing proposal…</> : "Generate Proposal"}
      </button>
