    ...
    <div style={{ fontFamily: N_FONT, color: N_FG, display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <textarea
              ...
              placeholder="Key takeaways, context, or anything worth remembering..."
              rows={3}
      ...
      {filtered.length === 0 ? (
        <div style={{
          ...
          background: N_SUBTLE, borderRadius: 12, border: `1px dashed ${N_BORDER}`,
        }}>
          <div style={{ fontSize: 32, marginBottom: 8 }}>📎</div>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>No documents yet</div>
          <div style={{ fontSize: 13, color: N_MUTED }}>
            Save links, notes, briefs, and specs — keep everything linked to the right project.
          </div>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    ...
                    {docType && (
                      <span style={{
                        ...
                        background: TYPE_COLORS[docType] ?? TYPE_COLORS["Other"],
                        color: TYPE_TEXT[docType] ?? TYPE_TEXT["Other"],
                      }}>
                        {docType}
                      </span>
                    )}
                  ...
                  {docNotes && (
                    <div style={{ fontSize: 13, color: N_MUTED, marginTop: 5, lineHeight: 1.5 }}>
                      {docNotes}
                    </div>
                  )}
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 6, flexWrap: "wrap" }}>
                    ...
                    {docDate && (
                      <span style={{ fontSize: 12, color: N_MUTED }}>{docDate}</span>
                    )}
                  </div>
                </div>
                <button
                  onClick={() => handleDelete(doc.pageId)}
                  disabled={deletingId === doc.pageId}
                  title="Delete"
                  style={{
                    background: "none", border: "none", cursor: "pointer",
                    color: N_MUTED, padding: 4, borderRadius: 6, flexShrink: 0,
                    opacity: deletingId === doc.pageId ? 0.4 : 1,
                  }}
                >
                  <Trash2 size={14} />
                </button>
